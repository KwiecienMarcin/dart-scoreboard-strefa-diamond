...
const tableNumberJokes = {
  pl: [
    (c) => `${c.player}, ${c.amount}? To akurat numer naszego stołu bilardowego, nie wynik w darta.`,
    (c) => `${c.player}, ${c.amount}? Zgaduję, że to numer naszego stolika bilardowego.`,
  ...
  en: [
    (c) => `${c.player}, ${c.amount}? That's actually one of our pool table numbers, not a darts score.`,
    (c) => `${c.player}, ${c.amount}? I'm guessing that's one of our pool table numbers.`,
...
const sportJokes = {
  pl: [
    ...
    { min: 30, max: 30, text: (c) => `${c.player}, ${c.amount}? Chyba ktoś tu gra dziś w tenisa.` },
    ...
    { min: 11, max: 11, text: (c) => `${c.player}, ${c.amount}? Tylu zawodników ma drużyna piłkarska.` },
    { min: 13, max: 13, text: (c) => `Pechowa trzynastka, ${c.player}. Chyba się dziś sprawdza.` },
    { min: 18, max: 18, text: (c) => `${c.player}, ${c.amount}? Tyle lat trzeba mieć, żeby zamówić piwo.` },
    { min: 25, max: 25, text: (c) => `${c.player}, ${c.amount}? Tyle trzeba w siatkówce, żeby wygrać seta.` },
    { min: 147, max: 147, text: (c) => `${c.amount}? Maksymalny break w snookerze, ${c.player}.` },
    { min: 44, max: 44, text: (c) => `${c.amount}? Numer Hamiltona w F1, ${c.player}.` },
    ...
    { min: 32, max: 32, text: (c) => `${c.player}, ${c.amount}? Tyle liter ma polski alfabet.` },
    { min: 52, max: 52, text: (c) => `${c.amount}? Talia do pokera ma tyle kart, ${c.player}.` },
    { min: 50, max: 50, text: (c) => `${c.player}, ${c.amount}? Tyle wynosi ograniczenie prędkości w mieście.` },
  ],
  en: [
    ...
    { min: 30, max: 30, text: (c) => `${c.player}, ${c.amount}? Someone's playing tennis tonight, not darts.` },
    ...
    { min: 11, max: 11, text: (c) => `${c.player}, ${c.amount}? That's a football team's size.` },
    { min: 13, max: 13, text: (c) => `Unlucky thirteen, ${c.player}. Checks out today.` },
    { min: 18, max: 18, text: (c) => `${c.player}, ${c.amount}? That's the legal drinking age here.` },
    { min: 25, max: 25, text: (c) => `${c.player}, ${c.amount}? That's a volleyball set score.` },
    { min: 147, max: 147, text: (c) => `${c.amount}? Maximum break in snooker, ${c.player}.` },
    { min: 44, max: 44, text: (c) => `${c.amount}? Hamilton's number in F1, ${c.player}.` },
    ...
    { min: 32, max: 32, text: (c) => `${c.player}, ${c.amount}? That's the Polish alphabet's size.` },
    { min: 52, max: 52, text: (c) => `${c.amount}? A deck of cards has that many, ${c.player}.` },
    { min: 50, max: 50, text: (c) => `${c.player}, ${c.amount}? That's the city speed limit here.` },
  ],
};
